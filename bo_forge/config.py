    ...
    *,
    ...
    if multi_objective:
        raise ConfigError(
            ...
            "campaigns configured with bo.acquisition: log_ei in v2.1.2; "
    ...
    if fidelity is not None:
        raise ConfigError(
            ...
            "in v2.1.2; use model.profile: default."
        )
    if stages:
        raise ConfigError(
            "Non-default model profiles cannot be combined with structured campaign "
            "stages in v2.1.2; use model.profile: default."
        )
    if bo.acquisition != "log_ei":
        raise ConfigError(
            "Non-default model profiles require bo.acquisition: log_ei in v2.1.2."
        )
