...
def test_manifest_uses_expected_release_directives() -> None:
    ...
    expected = {
        ...
        "recursive-include notebook_assurance *.py",
        ...
        "recursive-include tests/fixtures/cli_json *.json",
    }
...
def test_v3_docs_describe_current_fail_closed_provenance_resume() -> None:
    ...
    assert "## v3.3.4 - Stable Performance Evidence" in changelog
    assert f"## v{PROJECT_VERSION} - Structured Suggestion Dry-Runs" in changelog
    assert "## v3.4.0 - Versioned CLI JSON And Inspection Contracts" in changelog
