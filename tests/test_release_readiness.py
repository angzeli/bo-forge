...
def test_v1_5_0_docs_describe_correct_release_scope() -> None:
    readme = (PROJECT_ROOT / "README.md").read_text(encoding="utf-8")
    ...
    assert "backward compatible with the v1.4.3 multi-fidelity Streamlit baseline" in readme
    ...
    assert "v1.5.2 adds Streamlit creation support for contextual BO" in streamlit_app_docs
...
def test_v1_5_roadmap_line_is_active_after_v1_4_closeout() -> None:
    ...
    assert "Current baseline: `v1.5.2`" in roadmap
    ...
    assert "`v1.5.2` | Patch | Streamlit creation and suggestion controls" in roadmap
    ...
    assert re.search(
        r"## 🌐 v1\.5 - Contextual BO\s+Status: active",
        roadmap,
    )
...
def test_requirements_lock_matches_current_release_snapshot() -> None:
    ...
    assert "BO Forge v1.5.2" in requirements_lock
...
def test_installation_tutorial_covers_pip_install_paths() -> None:
    ...
    assert "dist/bo_forge-1.5.2-py3-none-any.whl" in tutorial
    assert "dist/bo_forge-1.5.2.tar.gz" in tutorial
...
def test_built_distributions_install_from_outside_source_tree(tmp_path: Path) -> None:
    ...
    wheels = sorted(dist_dir.glob("bo_forge-1.5.2-*.whl"))
    sdists = sorted(dist_dir.glob("bo_forge-1.5.2.tar.gz"))
    assert wheels, "No v1.5.2 wheel was built."
    assert sdists, "No v1.5.2 sdist was built."
...
def _assert_wheel_package_boundaries(wheel_path: Path) -> None:
    with zipfile.ZipFile(wheel_path) as wheel:
        ...
        metadata = wheel.read("bo_forge-1.5.2.dist-info/METADATA").decode("utf-8")
    ...
    assert "bo_forge-1.5.2.dist-info/entry_points.txt" in names
    assert "bo_forge-1.5.2.dist-info/licenses/LICENSE" in names
...
def _assert_sdist_contains_release_assets(sdist_path: Path) -> None:
    ...
    assert "bo_forge-1.5.2/README.md" in names
    assert "bo_forge-1.5.2/ROADMAP_V0_TO_V1.md" in names
    assert "bo_forge-1.5.2/ROADMAP_V1_X.md" in names
    assert "bo_forge-1.5.2/docs/PUBLIC_API.md" in names
    assert "bo_forge-1.5.2/docs/STREAMLIT_DEPLOYMENT.md" in names
    assert "bo_forge-1.5.2/docs/API_PROBE.md" in names
    assert "bo_forge-1.5.2/examples/quickstart.py" in names
    assert "bo_forge-1.5.2/examples/01_simple_2d_maximise_logei_campaign_log.csv" in names
    assert (
        "bo_forge-1.5.2/examples/10_multi_objective_mixed_constrained_campaign_log.csv"
        in names
    )
    assert (
        "bo_forge-1.5.2/examples/11_four_objective_mixed_constrained_campaign_log.csv"
        in names
    )
    assert "bo_forge-1.5.2/examples/12_cost_aware_multi_objective_campaign_log.csv" in names
    assert "bo_forge-1.5.2/examples/13_structured_campaign_core_campaign_log.csv" in names
    assert "bo_forge-1.5.2/examples/14_structured_campaign_tutorial_campaign_log.csv" in names
    assert "bo_forge-1.5.2/examples/15_multi_fidelity_qmfkg_campaign_log.csv" in names
    assert "bo_forge-1.5.2/examples/16_contextual_logei_campaign_log.csv" in names
    assert "bo_forge-1.5.2/configs/10_multi_objective_mixed_constrained_qlogehvi.yaml" in names
    assert "bo_forge-1.5.2/configs/11_four_objective_mixed_constrained_qlogehvi.yaml" in names
    assert "bo_forge-1.5.2/configs/12_cost_aware_multi_objective_qlogehvi.yaml" in names
    assert "bo_forge-1.5.2/configs/13_structured_campaign_core.yaml" in names
    assert "bo_forge-1.5.2/configs/14_structured_campaign_tutorial.yaml" in names
    assert "bo_forge-1.5.2/configs/15_multi_fidelity_qmfkg.yaml" in names
    assert "bo_forge-1.5.2/configs/16_contextual_logei.yaml" in names
    assert "bo_forge-1.5.2/notebooks/01_maximisation_logei_campaign.ipynb" in names
    assert "bo_forge-1.5.2/notebooks/10_multi_objective_qlogehvi_campaign.ipynb" in names
    assert "bo_forge-1.5.2/notebooks/11_four_objective_qlogehvi_campaign.ipynb" in names
    assert "bo_forge-1.5.2/notebooks/12_cost_aware_multi_objective_qlogehvi_campaign.ipynb" in names
    assert "bo_forge-1.5.2/notebooks/14_structured_campaign_tutorial.ipynb" in names
    assert "bo_forge-1.5.2/notebooks/15_multi_fidelity_qmfkg_campaign.ipynb" in names
    assert "bo_forge-1.5.2/notebooks/16_contextual_logei_campaign.ipynb" in names
...
def _install_distribution_and_probe(
    *,
    ...
    assert completed.stdout == "bo-forge 1.5.2\n"
    subprocess.run(
...
assert bo_forge.__version__ == "1.5.2"
