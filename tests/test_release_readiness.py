...
def test_installation_tutorial_covers_pip_install_paths() -> None:
    ...
    assert "dist/bo_forge-1.0.0-py3-none-any.whl" in tutorial
    assert "dist/bo_forge-1.0.0.tar.gz" in tutorial
...
def test_built_distributions_install_from_outside_source_tree(tmp_path: Path) -> None:
    ...
    wheels = sorted(dist_dir.glob("bo_forge-1.0.0-*.whl"))
    sdists = sorted(dist_dir.glob("bo_forge-1.0.0.tar.gz"))
    assert wheels, "No v1.0.0 wheel was built."
    assert sdists, "No v1.0.0 sdist was built."
...
def _assert_wheel_package_boundaries(wheel_path: Path) -> None:
    with zipfile.ZipFile(wheel_path) as wheel:
        ...
        metadata = wheel.read("bo_forge-1.0.0.dist-info/METADATA").decode("utf-8")

    assert "bo_forge/__init__.py" in names
    assert "bo_forge_app/streamlit_app.py" in names
    assert "bo_forge_app/cli.py" in names
    assert "bo_forge-1.0.0.dist-info/entry_points.txt" in names
    assert "bo_forge-1.0.0.dist-info/licenses/LICENSE" in names
...
def _assert_sdist_contains_release_assets(sdist_path: Path) -> None:
    ...
    assert "bo_forge-1.0.0/README.md" in names
    assert "bo_forge-1.0.0/ROADMAP_PRE_V1.md" in names
    assert "bo_forge-1.0.0/ROADMAP_AFTER_V1.md" in names
    assert "bo_forge-1.0.0/docs/PUBLIC_API.md" in names
    assert "bo_forge-1.0.0/docs/assets/streamlit_campaign_panel.png" in names
    assert "bo_forge-1.0.0/examples/quickstart.py" in names
    assert "bo_forge-1.0.0/examples/01_simple_2d_maximise_logei_campaign_log.csv" in names
    assert "bo_forge-1.0.0/notebooks/01_maximisation_logei_campaign.ipynb" in names
...
def _install_distribution_and_probe(
    *,
    ...
    assert completed.stdout == "bo-forge 1.0.0\n"
...
assert bo_forge.__version__ == "1.0.0"
