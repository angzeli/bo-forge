...
range or ordered numeric fidelity levels. v2.4.3 supports qMFKG batches
...
variables and optional defaults. In v2.4.3, single-objective contextual
...
with `bo.acquisition: log_ei` or `qlog_nei` in v2.4.3; multi-objective, multi-fidelity, and
...
BO Forge supports coupled multi-objective campaigns with `m >= 2` objectives. The primary tested range for v2.4.3 is `2 <= m <= 4`; larger objective counts are advanced usage because qLogEHVI/qLogNEHVI, non-dominated partitioning, hypervolume, and visualization become more expensive.
