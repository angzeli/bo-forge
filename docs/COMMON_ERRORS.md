...
v2.5.2 contextual BO support allows single-objective `bo.acquisition: log_ei`
...
v2.3.x model profiles are deliberately conservative.
...
v2.3.x qLogNEI support is deliberately conservative.
...
v2.5.2 qLogNEHVI support is deliberately narrow. It is only supported for
coupled multi-objective campaigns with `2 <= m <= 4`.

Fix: remove `cost:`, `replicates:`, `stages:`, `context:`, or `fidelity:`
from qLogNEHVI configs, or use `bo.acquisition: qlog_ehvi` for deterministic
coupled multi-objective campaigns. `source=qlog_nehvi` is valid only for
qLogNEHVI configs. See
[QLOGNEHVI_FEASIBILITY.md](QLOGNEHVI_FEASIBILITY.md).
...
v2.5.2 multi-fidelity support remains deliberately conservative.
...
v2.5.2 supports qMFKG batches up to four candidates. Larger batches are
