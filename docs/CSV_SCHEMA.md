...
context values. `stages:` cannot be combined with `cost:` in v2.5.1.
...
- `stages:` cannot be combined with `cost:` in v2.5.1.
...
`fidelity:` section. Ordered numeric levels remain supported in v2.5.1:
...
- when supplied, `fidelity.levels` must contain at least two finite, strictly
  increasing in-bounds values, must be separated enough for unambiguous
  `1e-9`-tolerance matching, and the highest level must equal the target;
- every populated CSV fidelity value must match exactly one configured level
  within the existing `1e-9` numeric tolerance;
...
- `fidelity_coverage()` and the `fidelity-progress` plot are read-only views of
  these existing rows and add no CSV columns; continuous coverage keeps exact
  user-space fidelity values as separate sorted rows, while discrete coverage
  maps each row to exactly one configured level;
...
v2.5.1 supports a conservative single-objective contextual LogEI/qLogEI workflow:
...
- single-objective contextual `bo.acquisition: log_ei` campaigns may combine
  `context:` with review metadata, deterministic `cost:`, replicates, or all
  three;
...
- contextual replicate groups retain one full design tuple, including context
  values. Group-mean fitting uses groups from all contexts, while
  `uncertain_best` active repeats only consider groups matching the requested
  normalized context;
- `context_summary()` counts CSV rows by context, including replicate rows;
  `replicate_summary()` remains one row per observed replicate group;
...
combined with `objectives:`, `stages:`, `fidelity:`,
...
BO Forge supports `m >= 2` objectives with coupled evaluation. The primary tested range for v2.5.1 is `2 <= m <= 4`; larger objective counts are advanced usage because qLogEHVI, non-dominated partitioning, hypervolume, and visualization become more expensive.
...
Review, replicate, and deterministic cost metadata are supported for multi-objective campaigns. Multi-objective cost-aware ranking uses qLogEHVI batch utility; cost is not modeled as another objective.
...
- Multi-objective replicate campaigns use group means plus replicate-derived `train_Yvar` for qLogEHVI fitting. Active repeat selection remains single-objective only in v2.5.1, so MO replicate configs default to `suggestion_policy: new_only` and explicit `uncertain_best` fails clearly.
...
`model.profile` changes only model-fitting behavior for supported single-objective campaigns configured with `bo.acquisition: log_ei` or `qlog_nei`. It does not add or remove CSV columns.
The canonical single-objective columns remain:
...
and structured campaigns in v2.5.1.
