...
BO Forge v3.3.3 provides a local Streamlit workbench around the existing `CampaignSession` workflow.

The v3.3.2 [benchmark harness](BENCHMARKS.md) is a separate source-only command
...
BO Forge coordinates Streamlit CSV mutations with CLI, notebook, and API
...
The v2.2 line includes model-profile visibility for supported single-objective campaigns.
The create flow can write `model.profile` for standard single-objective
campaigns configured with `bo.acquisition: log_ei`; loaded `qlog_nei` configs
