...
v0.2 adds a `CampaignSession` notebook engine: define a problem, load a CSV log, validate and summarise campaign state, suggest experiments, enter results, reload from disk, and plot progress through one reusable object.
...
The primary dependency source is `pyproject.toml`. A direct-dependency snapshot from the v0.2.0 environment is recorded in `requirements-lock.txt`.
